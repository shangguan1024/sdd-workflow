...
5. AI calls gate tool for approval
...
- Always call `（当前 Phase 技能由 hook 自动注入，见 SKILL.md 命令对照表）` first to determine actual primary skill
...
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" start user-auth
...
**Skill Override Rule:** Same as Phase 0 — if `skills` field configured → replaces default; otherwise → use default_primary_skills. Always call `（当前 Phase 技能由 hook 自动注入，见 SKILL.md 命令对照表）` first.
...
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" gate 2 check
...
# Request approval
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" gate 2 approve
# Wait for user confirmation
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" gate 2 approve --confirmed true
```
...
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" gate 3 check
...
# Wait for user confirmation
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" gate 3 approve --confirmed true
```
...
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" gate 4 check
...
# Wait for user confirmation
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" gate 4 approve --confirmed true

# Context refresh (if needed)
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" refresh AI deviating from design
```
...
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" gate 5 check
...
# Wait for user confirmation
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" gate 5 approve --confirmed true
```
...
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" gate 6 check
...
# Wait for user confirmation
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" gate 6 approve --confirmed true
```
...
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" gate 7 check
...
# Complete workflow
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" complete user-auth
...
3. Developer explicit confirmation received? (gate approve)
4. If ANY answer is NO → STOP (Plugin blocks tools)
```

**Human-in-Loop:**
- User must review phase output
- User must explicitly confirm via `gate approve`
...
```
# 1. Check status
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" status --verbose
...
# 2. Resume feature
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" resume user-auth
...
```
# Manual refresh
node "__SDD_CLAUDE_DIR__/engine/bin/sdd.js" refresh AI deviating from design document
